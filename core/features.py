...
def harmonic_benchmark(field: dict, m_eff: float, energies_meV: list[float]) -> dict:
    ...
    if field["dim"] == 1:
        ...
        y = np.asarray(field["y_nm"], dtype=float)
        V = np.asarray(field["V_eV"], dtype=float)
        shape_features = _features_2d(field)
        if shape_features.get("ring_like"):
            return {"applicable": False, "reason": "mínimo anular: un modelo armónico local 2D no describe el splitting angular"}
        ...
        dx = (x[1] - x[0]) * NM
        dy = (y[1] - y[0]) * NM
        local = V[max(0, iy - 2):iy + 3, max(0, ix - 2):ix + 3]
        span = float(np.nanmax(V) - np.nanmin(V))
        if span > 0 and np.count_nonzero(np.abs(local - V[iy, ix]) < max(1e-12, 1e-6 * span)) >= 5:
            return {"applicable": False, "reason": "fondo plano o discontinuo: la curvatura por diferencias finitas no representa ħω"}
